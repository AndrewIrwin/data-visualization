...
```{r}
gapminder %>% ggplot(aes(x = gdpPercap, y = lifeExp, color = continent)) + geom_point()
```
...
```{r}
gapminder %>% ggplot(aes(x = gdpPercap, y = lifeExp)) + geom_point(olour = "blue")
```
...
* geom_point, geom_line, geom_histogram, geom_boxplot, geom_bar, geom_text, geom_smooth, geom_jitter
...
```{r}
gapminder %>% ggplot(aes(x = gdpPercap, y = lifeExp, group = country, colour = continent)) + geom_line() +
  scale_x_log10()
...
```{r}
gapminder %>% ggplot(aes(x = lifeExp, y = continent)) + geom_boxplot()
```


```{r}
gapminder %>% ggplot(aes(x = lifeExp, y = continent)) + geom_jitter() + geom_boxplot()
gapminder %>% ggplot(aes(x = lifeExp, y = continent)) + geom_boxplot() + geom_jitter(height = 0.2, color = "gray50", alpha = 0.5) 

```

```{r}
gapminder %>% ggplot(aes(x = lifeExp, y = continent)) + stat_summary() + xlim(27, 85)

```

Example from `?geom_line`

```{r}
ggplot(economics, aes(date, unemploy)) + geom_line(size = 1.3, linetype = 5)
```
...
penguins %>% ggplot(aes(x = body_mass_g, fill = species)) + geom_histogram(binwidth = 225)
```
...
```{r}
gapminder %>% ggplot(aes(x = lifeExp, fill = factor(year))) + geom_histogram()
```
...
```{r}
gapminder %>% ggplot(aes(y = lifeExp, x = year, color = continent)) + stat_summary()
```
...
```{r}
gapminder %>% ggplot(aes(x = year, y = lifeExp, color = continent)) + geom_smooth() + geom_jitter(width = 0.2)
```

## Thinking about visualizations

Compare this scatterplot with `geom_jitter` and `geom_point`. What is the difference? Which is better? Why? Is one misleading?

```{r}
mpg %>% ggplot(aes(x = cty, y = hwy)) + geom_jitter()
```
