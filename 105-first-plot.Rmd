...
In this short lesson you will learn how to use ggplot to visualize data. There will be one simple example with several exercises, designed to make the essentials clear in your mind. All plots share the three features explained below, but many plots we draw later in the course are much more complex. 
...
We're going to revise the plot by building on these instructions. I'll repeat the instructions above, but instead of displaying the figure, I will store the result in a variable (conventionally called `p` for plot to keep it simple, but you can use any name you like.)
...
Add better labels to your plot using `labs` and `x`, `y`, `title`, `subtitle`, `tg`. My example has placeholder text. Revise the figure to show sensible text in these spaces.
...
This formatting of the numbers on the x-axis is a bit trick to get right and to explain. In fact its the sort of thing you might keep in a file of examples if, like me, you really don't like to see $10^3$ formatted as $1.0e3$. (Try `labels = scientific` to see this other format.)
